...
from common import (
    pack, unpack, b64encode_bytes, b64decode_str, ensure_downloads_dir,
    temp_file_path, open_with_default, MAX_FILE_BYTES, SMALL_INLINE_LIMIT,
    CHUNK_SIZE, gen_nonce, pbkdf2, derive_session_key,
    aes_available, aes_encrypt, aes_decrypt
)
...
class TCPClient:
    def __init__(self, host, port, username, on_event, password="password"):
        ...
        self.password = password; self.on_event = on_event
        self.sock = None; self.buf = b""; self.running = False
    ...
    def _send_plain(self, obj):
        try: self.sock.sendall(pack(obj))
    ...
    def _send_enc(self, inner_obj):
        if self.session_key is None or not aes_available():
            self._send_plain(inner_obj); return
        try:
            data = json.dumps(inner_obj, ensure_ascii=False).encode("utf-8")
            nonce, ct, tag = aes_encrypt(data, self.session_key)
            obj = {"enc":True,
    ...
    def send(self, obj): self._send_enc(obj)
    ...
    def share_file(self, path, thumb_b64=None):
        try: size = os.path.getsize(path)
            ...
            self.on_event({"type":"system","text":f"File access error: {e}"}); return
        if size > MAX_FILE_BYTES:
            self.on_event({"type":"system","text":"File too large (limit exceeded)."}); return

        if size <= SMALL_INLINE_LIMIT:
            try:
                with open(path,"rb") as f: inline_b64 = b64encode_bytes(f.read())
                self.send({"type":"file_offer","filename":os.path.basename(path),"size":size,
                           "inline_b64":inline_b64,"thumb_b64":thumb_b64})
            except Exception as e:
                self.on_event({"type":"system","text":f"File read/send error: {e}"})
        else:
            nonce = gen_nonce(); self._nonce_path[nonce] = path
            self.send({"type":"file_offer","filename":os.path.basename(path),"size":size,
                       "thumb_b64":thumb_b64,"nonce":nonce})

    def request_file(self, offer_id: str, mode: str="download"):
        if mode not in ("download","preview"): mode = "download"
            ...
            while True:
                ...
                if obj.get("enc") and self.session_key is not None and aes_available():
                    try:
                        ...
                        plain = aes_decrypt(nonce, ct, tag, self.session_key)
                ...
                # ---------------- AUTH FLOW ----------------
                if t == "auth_salt":
                    # Server sends salt for password-based key derivation
                    ...
                    try:
                        ...
                        pw_hash_b64 = base64.b64encode(pw_hash).decode("ascii")
                        # Send proof
                        self._send_plain({
                            "type": "auth_proof",
                    ...
                    try:
                        ...
                        self.on_event({"type": "system", "text": f"Auth parse error: {e}"})
                        continue
                    
                    base_key = self._last_pw_hash or pbkdf2(self.password, b"fallback")
                
                    try:
                        # derive only the TCP key; UDP key already sent in full form
                        self.session_key = derive_session_key(base_key, ssalt)
                
                        # forward UDP key to UI
                        if udp_key_b64:
                            self.on_event({
                                "type": "udp_key",
                                "key": udp_key_b64,
                ...
                # ---------------- FILE EVENTS ----------------
                    ...
                    if not path:
                        ...
                        continue
                    threading.Thread(target=self._stream_file_thread, args=(offer_id, path, 0), daemon=True).start()
                    ...
                    try:
                        ...
                        if mode == "preview":
                            ...
                            self.on_event({
                                "type": "system",
                                "text": f"Preview {'opened' if ok else 'saved'}: {out}"
                            })
                        else:
                            self.on_event({
                                "type": "system",
                                "text": f"Downloaded: {out}"
                            })
                    except Exception as e:
                    ...
                    self.on_event({
                        "type": "system",
                        "text": f"Downloading {oid}: {bso}/{size} bytes ({pct:.1f}%)"
                    })
    ...
    def _stream_file_thread(self, offer_id, path, offset=0):
        try:
            with open(path,"rb") as fh:
                if offset: fh.seek(offset)
                while True:
                    c = fh.read(CHUNK_SIZE)
                    if not c:
                        self._send_enc({"type":"file_chunk","offer_id":offer_id,"data_b64":"","eof":True})
                        break
                    self._send_enc({"type":"file_chunk","offer_id":offer_id,"data_b64":b64encode_bytes(c),"eof":False})
                    time.sleep(0.001)
        except Exception as e:
